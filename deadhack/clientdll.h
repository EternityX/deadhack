...
public:
	...
	virtual void ComputeLighting( const Vec3_t &pt, const Vec3_t *pNormal, bool bClamp, Vec3_t &color, Vec3_t *pBoxColors = nullptr ) = 0;
	...
	virtual void ClientCmd_Unrestricted( const char *szCmdString, const char *newFlag = nullptr ) = 0; // 114, new flag, quick testing shows setting 0 seems to work, haven't looked into it.
	...
	virtual void SolidMoved( class IClientEntity *pSolidEnt, class ICollideable *pSolidCollide, const Vec3_t *pPrevAbsOrigin, bool accurateBboxTriggerChecks ) = 0;
	virtual void TriggerMoved( class IClientEntity *pTriggerEnt, bool accurateBboxTriggerChecks ) = 0;
	virtual void ComputeLeavesConnected( const Vec3_t &vecOrigin, int nCount, const int *pLeafIndices, bool *pIsConnected ) = 0;
	...
	virtual void SpherePaintSurface( const model_t *model, const Vec3_t &location, unsigned char chr, float fl1, float fl2 ) = 0;
	...
	virtual void SphereTracePaintSurface( const model_t *model, const Vec3_t &position, const Vec3_t &vec2, float radius, /*CUtlVec3_t<unsigned char, CUtlMemory<unsigned char, int>>*/ int &utilVecShit ) = 0;
